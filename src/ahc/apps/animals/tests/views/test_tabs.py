...
class TestAnimalTabView:
    ...
    def test_htmx_request_returns_fragment_without_base_title(self, animal, user_profile, logged_in_client):
        user, _ = user_profile
    ...
    def test_non_htmx_request_returns_full_page_with_base_title(self, animal, user_profile, logged_in_client):
        user, _ = user_profile
        c = logged_in_client(user)
        url = f"/pet/{animal.id}/tab/mainpage/"
        response = c.get(url)
    ...
    def test_all_public_slugs_return_200_for_owner(self, animal, user_profile, logged_in_client):
        user, _ = user_profile
        c = logged_in_client(user)
    ...
    def test_owner_only_tabs_return_403_for_keeper(self, animal, second_user_profile, logged_in_client):
        other_user, other_profile = second_user_profile
        animal.allowed_users.add(other_profile)
        c = logged_in_client(other_user)
    ...
    def test_non_accessible_user_gets_403(self, animal, second_user_profile, logged_in_client):
    ...
    def test_unknown_slug_returns_404(self, animal, user_profile, logged_in_client):
        user, _ = user_profile
        c = logged_in_client(user)
