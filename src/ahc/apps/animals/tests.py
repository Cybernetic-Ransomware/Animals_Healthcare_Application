...
class TestBiometricsTabAccess:
    ...
    def test_owner_sees_biometrics_tab_and_can_open_it(self, animal, user_profile):
        user, _ = user_profile
        c = self._client_for(user)
    ...
    def test_keeper_with_allow_biometrics_sees_tab(self, animal, second_user_profile):
        from ahc.apps.animals.models import AnimalShare

        carer_user, carer_profile = second_user_profile
        AnimalShare.objects.create(animal=animal, carer=carer_profile, allow_biometrics=True)
        c = self._client_for(carer_user)
        response = c.get(f"/pet/{animal.id}/tab/biometrics/", HTTP_HX_REQUEST="true")
        assert response.status_code == 200

    def test_keeper_without_allow_biometrics_blocked(self, animal, second_user_profile):
        from ahc.apps.animals.models import AnimalShare
    ...
    def test_expired_share_blocked(self, animal, second_user_profile):
        from ahc.apps.animals.models import AnimalShare
    ...
    def test_deceased_owner_sees_history_but_no_actions(self, user_profile):
        user, profile = user_profile
    ...
    def test_keeper_only_history_does_not_see_biometrics(self, animal, second_user_profile):
        from ahc.apps.animals.models import AnimalShare
    ...
    def test_keeper_only_biometrics_does_not_see_notes(self, animal, second_user_profile):
        from ahc.apps.animals.models import AnimalShare
...
class TestBiometricsTabData:
    ...
    def test_date_event_started_wins(self, animal, user_profile):
        user, profile = user_profile
    ...
    def test_fallback_to_localized_date_creation(self, animal, user_profile):
    ...
    def test_fallback_to_date_updated_when_related_note_is_none(self, animal, user_profile):
        user, profile = user_profile
    ...
    def test_weight_grouped_by_unit(self, animal, user_profile):
        user, profile = user_profile
    ...
    def test_chart_points_ascending_history_rows_descending(self, animal, user_profile):
        user, profile = user_profile
    ...
    def test_history_rows_tie_break_by_id_descending(self, animal, user_profile):
        user, profile = user_profile
    ...
    def test_custom_visible_in_history_but_not_charted(self, animal, user_profile):
        from ahc.apps.medical_notes.models.type_basic_note import MedicalRecord
        from ahc.apps.medical_notes.services.biometrics import create_biometric_record
    ...
    def test_empty_state_no_canvas_no_data_script(self, animal, user_profile):
        user, _ = user_profile
        response = self._get(self._client_for(user), animal)
        content = response.content.decode()
    ...
    def test_single_point_still_renders_chart_card(self, animal, user_profile):
        user, profile = user_profile
        _create_biometric_weight(animal, profile, 4.2, date_event_started=date(2026, 1, 1))
        response = self._get(self._client_for(user), animal)
        content = response.content.decode()
    ...
    def test_multiple_weight_units_render_separate_cards(self, animal, user_profile):
        user, profile = user_profile
        _create_biometric_weight(animal, profile, 4.2, unit="kg", date_event_started=date(2026, 1, 1))
        _create_biometric_weight(animal, profile, 4200, unit="g", date_event_started=date(2026, 1, 2))
        response = self._get(self._client_for(user), animal)
    ...
    def test_history_table_shows_value_and_unit(self, animal, user_profile):
        user, profile = user_profile
        _create_biometric_weight(animal, profile, 4.2, unit="kg", date_event_started=date(2026, 1, 1))
        response = self._get(self._client_for(user), animal)
        content = response.content.decode()
    ...
    def test_htmx_response_is_fragment_direct_navigation_is_full_shell(self, animal, user_profile):
        user, _ = user_profile
        c = self._client_for(user)
